...
def _print_results(results, min=0):
    ...
    for result in results:
        ...
        if result['amt'] >= min:
            total += result['amt']
            out.append('\t%s\t\t%s\t\t%s' % (locale.currency(result['amt']), result['date'], result['desc']))
    out.append('')

    if min > 0:
        print '\n'.join(out)
        print len(out) - 2, 'purchases over %s, sum %s' % (locale.currency(min), locale.currency(total))
    print len(results), 'purchases total, sum', locale.currency(gtotal)

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-d', '--date', type=int, help='specify YYYYMM to process')
    parser.add_argument('-m', '--min', type=int, help='specify min amount to show details')
